...
module Private : sig
  (** [push_upstream_log_sexp] acts like [sexp] except
      [Private.Message.is_from_upstream_log] is true. This can be useful e.g. if you have
      a separate log library that is both forwarding messages to [Async_log] to store them
      on disk, and collecting messages from [Async_log] via [set_output] to store
      somewhere else. In this case, [is_from_upstream_log] could be used to ignore
      messages coming from [Async_log] that originated from the upstream library. *)
  val push_upstream_log_sexp
    :  t
    -> Sexp.t
    -> level:Level.t
    -> time:Time.t
