...
module File : sig
  val create : ?perm:Unix.file_perm -> Output_format.t -> filename:string -> t
end = struct
...
module Log_writer : sig
  val create : Output_format.t -> Writer.t -> t
end = struct
...
module Rotating_file : sig
  ...
  module Make (Id : Rotation_id.S) = struct
      ...
      let%map (_ : unit Or_error.t list) =
        (match keep with
         ...
         | `Newer_than span ->
           ...
           let now = Time.now ~time_source:None in
    ...
    let rotate t ~time_source =
      ...
      t.last_size <- 0;
      t.last_messages <- 0;
      t.last_time <- Time.now ~time_source;
      t.filename <- filename;
      t.writer <- open_writer ~filename ~perm:t.perm
    ;;

    let write t ~time_source msgs =
      let current_time = Time.now ~time_source in
    ...
    let create ?perm ?time_source ~log_on_rotation format ~basename ~suffix rotation =
      ...
      let t_deferred =
        ...
        ; last_size = 0
        ; last_messages = 0
        ; last_time = Time.now ~time_source
        ; log_files
        ; log_on_rotation
        ; perm
        }
  ...
  module Timestamped = Make (struct
    ...
    let create ?time_source _zone = Time.now ~time_source
  ...
  module Dated = Make (struct
    ...
    let create ?time_source zone = Date.of_time (Time.now ~time_source) ~zone
    let rotate_one = Fn.id
